...
def runner_hand_3d_joints_ransac_torch(
    ...
    Ms_tensor = torch.from_numpy(Ms).to(dtype=torch.float32, device="cuda")
    handmarks = torch.from_numpy(mp_handmarks).to(dtype=torch.float32, device="cuda")
    num_hands, num_cams, joints, _ = handmarks.shape

    hand_joints_3d = torch.full((num_hands, joints, 3), -1, dtype=torch.float32)
    for hand in range(num_hands):
        ...
        b_pts_3d = triangulate_handmarks_batch(
            ...
            Ms_tensor[combinations[:, 0]],
            Ms_tensor[combinations[:, 1]],
        )
...
def runner_draw_handmarks_results(rgb_images, handmarks, serials):
    ...
    return display_images(vis_image, serials, return_array=True)
...
class ManoJointsEstimator:
    ...
    def run_hand_joints_3d_estimation(self):
        ...
        hamer_handmarks_file = self._save_folder / "hamer_handmarks_results.npz"
        if not hamer_handmarks_file.exists():
            self._logger.error(
                "    ** Hand 2D Joints Detection Results does not exist!!!"
            )
            return

        # Load hamer handmarks
        hamer_handmarks = np.load(hamer_handmarks_file)
        hamer_handmarks = np.stack(
            [hamer_handmarks[s] for s in self._serials], axis=2
        )  # (num_hands, num_frames, num_cameras, num_joints, 2)
        self._logger.info(f"    ** hamer_handmarks loaded: {hamer_handmarks.shape}")

        joints_3d_file = self._save_folder / "hamer_hand_joints_3d_raw.npy"
        if joints_3d_file.exists():
            ...
            self._logger.info("    ** Estimating 3D hand joints by RANSAC...")
            ...
            self._logger.info("    ** Saving 3D hand joints estimation results...")
            np.save(joints_3d_file, hand_joints_3d)

        # Complete missing 3D joints by interpolation
        self._logger.info("    ** Completing missing 3D joints by interpolation...")
        ...
        self._logger.info("    ** Saving completed 3D hand joints...")
        np.save(
            self._save_folder / "hamer_hand_joints_3d_interpolated.npy", hand_joints_3d
        )

        self._logger.info("    ** Saving 3D hand joints projection...")
        ...
        np.savez_compressed(
            self._save_folder / "hamer_hand_joints_3d_projection.npz",
            **{
        ...
        save_vis_folder = self._save_folder / "vis" / "hamer_hand_joints_3d"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HaMeR Hand 3D Joints Estimation")
    parser.add_argument(
    ...
    pose_estimator = ManoJointsEstimator(sequence_folder=sequence_folder)
